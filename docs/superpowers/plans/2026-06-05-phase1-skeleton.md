...
**Goal:** 实现能跑通 PUB → SUB 端到端的最小 Broker，不含认证/权限/通配符/批处理/背压/监控。
...
git commit -m "实现配置模块：BrokerConfig 数据类 + 环境变量覆盖"
```
...
@pytest.fixture
async def broker_config(tmp_path):
    """随机端口的 Broker 配置。"""
    import random
