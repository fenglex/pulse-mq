#!/usr/bin/env python3
"""100k 消息压测 (启用客户端 Batcher)。

仿照 bench_baseline.py, 但 PulseClient 启用批量:
- batch_size=10, batch_interval_ms=10, batch_max_wait_ms=50
- 测试 16 组合
- 100k 消息每组合
- 测吞吐, p50, p99
- 写到 docs/perf-100k-batched-data.md

用法:
    PYTHONIOENCODING=utf-8 uv run python scripts/bench_100k_with_batcher.py \
        --port 16900 --output docs/perf-100k-batched-data.md --n-messages 100000
"""
...
async def bench_one(port: int, data_type: str, comp: str, n_messages: int) -> dict:
    ...
    与 bench_baseline.py 的区别: 客户端启用 Batcher.
    """
    address = f"tcp://localhost:{port}"
    xpub_address = f"tcp://localhost:{port + 1}"
    latencies: list[float] = []
    ser = _ser_for(data_type)
    topic = f"bench.{data_type}.{comp}.batched"
    ...
    async def pub() -> None:
        async with PulseClient(
            ...
            for i in range(n_messages):
                ...
                # 启用 Batcher 后不必手动 yield (Batcher 内部已批量发送)
                # 但仍每 200 条让出一次, 防止 sub 饥饿
                if i > 0 and i % 200 == 0:
    ...
    try:
        ...
        await asyncio.wait_for(pub(), timeout=timeout_s)
        try:
...
def write_markdown(results: list[dict], output: str, n_messages: int) -> None:
    ...
    with open(output, "w", encoding="utf-8") as f:
        f.write("# PulseMQ 100k 批量压测数据\n\n")
        f.write(f"**测试环境**: Python {sys.version.split()[0]}, ")
        f.write(f"{sys.platform}\n\n")
        f.write(
            "**场景**: 1 pub + 1 sub (同机 loopback), 客户端启用 Batcher, "
            f"每组合 N 条消息, pub 顺序发布、sub 异步接收。\n\n"
        )
        f.write(
            f"**Batcher 配置**: batch_size={BATCH_SIZE}, "
            f"batch_interval_ms={BATCH_INTERVAL_MS}, "
            f"batch_max_wait_ms={BATCH_MAX_WAIT_MS}\n\n"
        )
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="PulseMQ 100k 批量压测")
    parser.add_argument("--port", type=int, default=16900, help="server 端口 (默认 16900)")
    parser.add_argument(
        "--output", type=str, default="docs/perf-100k-batched-data.md",
        help="markdown 输出路径",
    )
    parser.add_argument(
        "--n-messages", type=int, default=N_MESSAGES,
        help=f"每组合消息数 (默认 {N_MESSAGES})",
    )
    args = parser.parse_args()

    print(f"=== PulseMQ 100k 批量压测 ===")
    print(f"  端口:     {args.port}/{args.port + 1}")
    print(f"  每组合:   {args.n_messages} 条")
    print(f"  组合数:   {len(DATA_TYPES) * len(COMPRESSIONS)}")
    print(f"  Batcher:  size={BATCH_SIZE}, interval={BATCH_INTERVAL_MS}ms, max_wait={BATCH_MAX_WAIT_MS}ms")
